...
class ImageBuilder(ABC):
    def __init__(self, args, config, td):
        self.tempdir = td
        self.args = args
        self.config = config
...
class FlaskImageBuilder(ImageBuilder):
    ...
    def copy_wrapper_files(self):
        ...
        wrapper_requirements = render_template(self.config['mode'],
                                               'wrapper-requirements.txt')

        m = re.search(r"(?P<module>.*).(?P<extension>py)$",
                      self.config['interface']['file']).groupdict()
        ...
        with open(os.path.join(self.tempdir, "wrapper_requirements.txt"), "w") as f:
    ...
    def copy_client_files(self):
        ...
        for file_or_folder in os.listdir(self.args.dir):
            ...
            if os.path.isdir(file_or_folder):
                ...
                copytree(file_or_folder, os.path.join(self.tempdir, file_or_folder))
            else:
                if not os.path.exists(self.tempdir):
                    os.mkdir(self.tempdir)
                copyfile(file_or_folder, os.path.join(self.tempdir, file_or_folder))
